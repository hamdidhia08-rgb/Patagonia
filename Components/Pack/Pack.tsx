...
import { Menu, X, Search, Mail, Play, Star, Heart, User, MapPin } from "lucide-react";
...
const RecommendationCard: React.FC<{ data: Recommendation }> = ({ data }) => {
  ...
  const formattedRating = Number.isInteger(rating) ? rating.toFixed(0) : rating.toFixed(1);

  return (
    <div className="bg-white rounded-xl shadow-lg overflow-hidden max-w-sm border border-gray-100 hover:shadow-xl transition-shadow duration-300 shadow-left-md">
...
const RecommendedSection: React.FC = () => {
  return (
    <section className={`py-12 px-4 md:px-8 bg-gray-50  ${barlow.className}`}>
      {/* Sous-titre général */}
      <p className={`${hand.className} text-center text-[#f1a435] text-lg md:text-4xl mb-2`}>Discover our top picks</p>
      {/* Titre principal */}
